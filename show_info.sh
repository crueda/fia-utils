...
if (len(sys.argv) < 5):
    print '$> show_info.sh caja stadistics_file fecha_inicial fecha_final'
    print 'EJEMPLO: $> ./show_info.sh 002 ../logs/Stadistics.log 2014-01-1508:15:00 2014-01-1508:17:10'
    exit()
					...
					if line.find('PRAVE')>0 :
						...
						if (index_separador>-1):
							...
							if box_number == str_box:                        
								rssi = vector_prave[12]
								voltage_raveon = vector_prave[10]
								satelites_raveon = vector_prave[7]
								fix_raveon = vector_prave[6]
								print hora_llegada + " -> " + "RSSI:" + rssi + " - SATS:" + satelites_raveon + " fix(" + fix_raveon  + ") - VOLTAGE:" + voltage_raveon
		except:
			pass
	except:
		pass